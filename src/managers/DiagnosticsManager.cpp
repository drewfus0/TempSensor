...
void DiagnosticsManager::printPeriodic(const SystemHealth& health) {
  Serial.printf(
      "[Diag] uptime=%lus free_heap=%lu largest_block=%lu graph=%u/%u logq=%u/%u dropped=%lu wifi=%s sd=%s ntp=%s\n",
      ...
      static_cast<unsigned int>(health.graphBufferUsage),
      static_cast<unsigned int>(health.graphBufferCapacity),
