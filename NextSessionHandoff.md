...
*   **3-Day Battery Graph, Solar Shading, and Coordinates Config:** Expanded the battery graph to show a combined **3-day timeline** (48 hours of history + 24 hours of future predictions). Historical data is drawn as solid lines, and future predictions are drawn as dashed lines. Integrated dynamic, offline day/night canvas shading and dotted sunrise/sunset lines with annotations onto the graph. Added **Latitude** and **Longitude** configuration fields to `/config.json`, the ESP8266 backend, and the settings dashboard to automatically drive these astronomical calculations client-side based on location.
